...
state_series_ids_gdp <- paste0(state_abbr, "RQGSP")
all_states_df <- data.frame()

for (i in seq_along(state_series_ids_gdp)) {
  state_data <- fredr(series_id = state_series_ids_gdp[i], frequency = "a")
...
write.csv(all_states_df, "all_states_rgdp.csv", row.names = FALSE)
